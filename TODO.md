...
- [x] `routing.py` — wrap `requests.get` in `try/except RequestException` for graceful degradation when Maps API is transient (2026-04-24)
- [x] `routing.py` — move `import hashlib` / `import math` to top-level imports (2026-04-24)
...
- [ ] `document-organizer` — after sorting, generate action items from documents: bills → "pay by [due date]", letters → "reply by [date]", contracts → "sign/return by [date]" etc. Include in the WhatsApp report as a task list
