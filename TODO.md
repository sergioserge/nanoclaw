...
- [ ] `organizer.py` — commit typo fix (`unssortiertFolderId` → `unsortiertFolderId`) and `prev_name` routing.py change
...
- [ ] `organizer.py` — replace bare `open(...)` with `with open(...) as f:` to avoid leaked file handles
...
- [ ] `SKILL.md` — add explicit hard rule: never access `documents.db` directly; never call Google API directly; all operations must go through `organizer.py`
- [ ] `organizer.py` `list_inbox` — add pagination (Google Drive returns max 100 per page; inbox with >100 files silently drops the rest)
